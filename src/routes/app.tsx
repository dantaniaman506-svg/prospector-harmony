...
export const Route = createFileRoute("/app")({
  head: () => ({
    meta: [
      { title: "Dashboard — AirLeads AI" },
      {
        name: "description",
        content:
          "AirLeads AI workspace: generate verified business leads that have no website, with phone, email and socials in seconds.",
      },
      { property: "og:title", content: "Dashboard — AirLeads AI" },
      {
        property: "og:description",
        content: "Generate verified leads with name, phone, email and socials in one tap.",
      },
...
function AppPage() {
  ...
  if (!authed) return <div className="min-h-dvh bg-background" />;
...
function Dashboard({ onSignOut }: { onSignOut: () => void }) {
  ...
  return (
    <div className="relative min-h-dvh pb-32 lg:pb-0 lg:pl-64">
      <TechBackground />
      <DesktopSidebar active={tab} onChange={setTab} onSignOut={onSignOut} />

      <header className="glass-panel sticky top-0 z-30 border-x-0 border-t-0 px-5 py-3 lg:px-10">
        <div className="mx-auto flex w-full max-w-md items-center justify-between gap-2 lg:max-w-6xl">
          <span className="font-display text-lg font-extrabold tracking-tight lg:hidden">
            AirLeads <span className="text-primary">AI</span>
          </span>
          <span className="hidden text-sm font-bold text-muted-foreground lg:block">
            {tabTitle(tab)}
          </span>
      ...
      <main className="mx-auto w-full max-w-md space-y-5 px-5 pt-5 lg:max-w-6xl lg:px-10 lg:pb-16 lg:pt-8">
        {tab === "dashboard" && (
          <>
            <div>
              <h1 className="text-[26px] font-extrabold leading-tight lg:text-4xl">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-5">
              {stats.map(({ label, value, Icon, series }) => (
                <div key={label} className="card-soft overflow-hidden p-4 lg:p-5">
                  ...
                  <p className="text-[22px] font-extrabold leading-tight lg:text-[28px]">{value}</p>
        ...
        {tab === "leads" && (
          <>
            <div className="flex items-end justify-between">
              <div>
                <h1 className="text-[24px] font-extrabold lg:text-3xl">Your Leads</h1>
        ...
        {tab === "history" && (
          <>
            <h1 className="text-[24px] font-extrabold lg:text-3xl">History</h1>
            {runs.length === 0 ? (
              <EmptyState
                title="No runs yet"
        ...
        {tab === "saved" && (
          <>
            <h1 className="text-[24px] font-extrabold lg:text-3xl">Saved</h1>
            {savedLeads.length === 0 ? (
              <EmptyState
                title="Nothing saved"
        ...
        {tab === "settings" && (
          <div className="lg:max-w-2xl">
            <h1 className="text-[24px] font-extrabold lg:text-3xl">Settings</h1>
            ...
            <button
              ...
              className="press mt-5 flex w-full items-center justify-center gap-2 rounded-full border-2 border-destructive/40 py-4 text-sm font-bold text-destructive"
            >
